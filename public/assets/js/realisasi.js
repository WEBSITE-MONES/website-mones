...
const dataTableOptions = {
    ...
    dom:
        "<'row mb-3'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6 d-flex justify-content-end'f>>" +
        "<'row'<'col-sm-12'tr>>" +
...
$(document).ready(function () {
    ...
    // Smooth scroll untuk tab navigation (hanya scroll ke tab header, bukan ke dalam tabel)
    $('button[data-bs-toggle="tab"]').on("shown.bs.tab", function (e) {
        const target = $(e.target).attr("data-bs-target");

        // Hanya scroll jika user sedang di bawah posisi tab
        const tabPosition = $(target).offset().top - 100;
        const currentScroll = $(window).scrollTop();

        // Hanya scroll jika tab di luar viewport
        if (
            currentScroll > tabPosition ||
            currentScroll < tabPosition - $(window).height()
        ) {
            $("html, body").animate(
                {
    ...
    $(document).on("click", ".dropdown-item.text-danger", function (e) {
        ...
        if (
            form.length &&
            form.attr("action") &&
            form.attr("action").includes("destroy")
        ) {
            ...
            if (typeof Swal !== "undefined") {
                Swal.fire({
                    ...
                    confirmButtonText:
                        '<i class="fa fa-trash me-2"></i>Ya, Hapus Semua!',
                ...
                if (
                    confirm(
                        "Apakah Anda yakin ingin menghapus data PR beserta PO, Progress, GR, dan Payment?\n\nData yang dihapus tidak dapat dikembalikan!"
                    )
                ) {
                    form[0].submit();
                }
